...
- [ ] **段階3 — Webリサーチ（SearXNG自ホスト）**: `searxng/` コンテナを `ollama_net` 内に追加（ホストポート非公開・内部専用）。`tools/web.py`（`search_web`/`fetch_url`、httpx）→ `web_research` サブタスクが検索→要約→ `scratchpad` に格納。`WEB_RESEARCH_ENABLED`。**Google エンジンは無効化**し DuckDuckGo/Brave/Startpage 等を使用（Googleは自ホストSearXNGをブロックするため）。
