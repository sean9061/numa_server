...
- [ ] **Phase 1.5 — Moodle連携**: 課題・締切をクロール対象に追加。**サイト全体が Google SSO ゲートウェイ(`login.service.cloud.teu.ac.jp`)の内側**で、WS APIトークン/フォームログイン/iCalトークンURL のいずれも(セッション無しでは)`accounts.google.com` へリダイレクトされ素通り不可と実機判明(site root・`webservice/rest/server.php`まで全部リダイレクト)。よって **ブラウザのログイン後セッションCookieを注入**(`MOODLE_COOKIE`)してカレンダー iCal エクスポートURL(`MOODLE_ICAL_URL`)を httpx 取得→VEVENTパースで課題締切を抽出(完全読み取り専用・追加依存なし・stdlibのみ)。gmail/gcalと同じ`{source:"moodle:..",link,title,due,course}`形で`crawl_node`に4つ目のソースとして合流→simple(reconcile)/orchestrator(integrateで確実に候補化)両経路に乗る。締切は明示値を最優先で流用。Cookieは失効するため、iCalでないレスポンス(=SSOリダイレクト)を `SessionExpired` として検知し実行サマリ(Discord)に「⚠Moodleセッション失効」を出す→手動でCookie更新。`MOODLE_ENABLED`(既定false)。**TLS注意**: 学校サーバは弱いDHE(1024bit)も提供しDH_KEY_TOO_SMALLで弾かれるため SSLコンテキストで `DEFAULT:!DHE`(ECDHE強制・証明書検証は維持)。**実装済み・Cookie設定待ち＋Cookie寿命を実測中**: `scripts/moodle_probe.py` で確認してから有効化する。寿命が短すぎれば棚上げ判断。
