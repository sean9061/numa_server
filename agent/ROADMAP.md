...
- [x] **段階B — Discord対話（司書）**（実装済み・smoke検証・本番反映は要 Message Content Intent）: `librarian.py`（`interpret()`＝会話→remember/forget/list/none）＋ `discordbot.on_message`＋ `_ConfirmView`（確認HITL）。会話で方針を覚える/忘れる/一覧。司書はエージェント本体と同一プロセスのため directive 更新は再起動なしに次回クロールへ反映。`LIBRARIAN_ENABLED`(既定true)。**要 Discord Developer Portal の Message Content Intent 有効化**。矛盾の自動supersedeは段階Cへ。
- [ ] **段階C — 運用**: 予算/優先度の昇格降格・矛盾解消・deterministic routing（硬い設定への振り分け）・コンパクション。
